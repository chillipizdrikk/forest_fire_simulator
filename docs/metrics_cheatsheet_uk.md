...
- `burned_components` — кількість звʼязаних компонент вигорілих клітин (8-звʼязність). Більше значення часто означає більш **фрагментовану** пожежу.
- `largest_cluster_share` — частка найбільшого кластера серед усіх вигорілих клітин (`largest_component_area / burnt_area`, `0..1`). Ближче до `1` = одна домінантна пляма вигорання.
- `shape_complexity` — проксі складності форми `perimeter / area` для вигорілої маски (4-контурний периметр). Вище = більш "рвана"/розгалужена геометрія фронту.
